      ...
      <div class="section">
        <div class="receipt-box">
            ...
            return groups.map((g, i) => html`
              <div key=${i} style="margin-bottom:${i < groups.length - 1 ? '12px' : '0'};padding-bottom:${i < groups.length - 1 ? '12px;border-bottom:1px dashed #3a5e5e' : '0'}">
                <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:6px">
                  ...
                  <span style="font-weight:700;color:#e8a84c;font-size:14px;white-space:nowrap;flex-shrink:0">${currency} ${fmt(currency, g.total)}</span>
                </div>
          ...
          <div style="text-align:right;margin-top:10px;padding-top:8px;border-top:1px solid #3a5e5e;font-size:12px;color:#8aaa9e">
            總計 <span style="font-weight:800;color:#e8a84c;font-size:15px;margin-left:4px">${currency} ${fmt(currency, result.total)}</span>
          </div>
        </div>
        ...
        <div class="summary-line">
          ${currency} ${fmt(currency, result.total)} ${t.total} · ${nSett} ${nSett>1?t.transfers:t.transfer} ${t.toSettle} <span class="check">✓</span>
        </div>
